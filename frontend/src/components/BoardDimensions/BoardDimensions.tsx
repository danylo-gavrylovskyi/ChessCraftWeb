...
const BoardDimensions: React.FC = () => {
  ...
  const handleNextPageClick = () => {
    setActiveGame(new Gameset({ ...activeGame, board: new ChessBoard(rows, columns) }))
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.content}>
        <div className={styles.controls}>
          <label>
            ...
            <input
              value={rows}
              onChange={(e) => !parseInt(e.target.value) ? setRows(0) : setRows(parseInt(e.target.value))}
          ...
          <label>
            ...
            <input
              value={columns}
              onChange={(e) => !parseInt(e.target.value) ? setColumns(0) : setColumns(parseInt(e.target.value))}
              className={styles.input}
            />
          </label>
        </div>
      ...
      <div className={styles.buttons}>
        <button className={styles.button} onClick={() => navigate('/')}><svg width="20%" viewBox="0 0 28 48" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          Prev</button>
        <button className={styles.button} onClick={handleNextPageClick}>Next <svg width="20%" viewBox="0 0 28 48" fill="none" xmlns="http://www.w3.org/2000/svg">
